...
import { ArrowLeft, Crown, Check, MessageCircle, Info } from "lucide-react";
...
const VIP_PLANS = [
  {
    months: 3,
    priceId: "price_1T0UXNBfuzQl0PTiKGR1SBTV",
    total: 297,
    monthly: 99,
    savings: null,
    label: "3 mois",
  ...
  {
    months: 6,
    priceId: "price_1T0UXRBfuzQl0PTir9qz5lUy",
    total: 495,
    monthly: 82.5,
  ...
  {
    months: 12,
    priceId: "price_1T0UXTBfuzQl0PTi6KK2azBu",
    total: 990,
    monthly: 82.5,
...
export default function VipCheckout() {
  ...
  const handlePayment = async () => {
    if (!discordId.trim()) {
      toast({
        title: "ID Discord requis",
        description: "Entre ton ID Discord pour recevoir le rôle VIP automatiquement.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (!session) {
        toast({
          ...
          description: "Tu dois être connecté pour souscrire au VIP.",
          variant: "destructive",
        });
        setLoading(false);
        return;
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-3 mb-8">
            {VIP_PLANS.map((p, i) => (
              ...
              >
                {p.savings && (
                  <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap">
                    {p.savings}
                  </span>
                )}
                ...
                <div className="text-xs text-muted-foreground">
                  soit {p.monthly.toFixed(p.monthly % 1 === 0 ? 0 : 1)}€/mois
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="bg-background rounded-xl border p-6 mb-8">
            ...
            <Label htmlFor="discord-id" className="text-sm text-muted-foreground mb-2 block">
              Nécessaire pour t'attribuer automatiquement le rôle VIP sur le serveur Discord.
            </Label>
            <Input
              id="discord-id"
            ...
            <div className="flex items-start gap-2 text-xs text-muted-foreground">
              ...
              <span>
                Pour trouver ton ID Discord : Active le mode développeur dans les paramètres Discord 
                (Paramètres &gt; Avancé &gt; Mode développeur), puis fais un clic droit sur ton profil 
                et clique "Copier l'identifiant".
              </span>
            </div>
          </div>
          ...
          <p className="text-xs text-center text-muted-foreground mt-4">
            Paiement sécurisé via Stripe. Tu seras redirigé vers la page de paiement.
          </p>
        </div>
      </Section>
    </Layout>
