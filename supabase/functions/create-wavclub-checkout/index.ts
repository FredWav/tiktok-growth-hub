...
/**
 * Plan metadata:
 *   acces  → 39€/mois  → Discord role @vip1  → env: STRIPE_PRICE_WAVCLUB_ACCES
 *   live   → 97€/mois  → Discord role @vip2  → env: STRIPE_PRICE_WAVCLUB_LIVE
 */
const PLAN_CONFIG = {
  acces: {
    envKey: "STRIPE_PRICE_WAVCLUB_ACCES",
    label: "Wav Club · Accès",
    discordRoleEnv: "DISCORD_VIP1_ROLE_ID",
  },
  live: {
    envKey: "STRIPE_PRICE_WAVCLUB_LIVE",
    label: "Wav Club · Live",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!priceId) {
      ...
      await notifyError("WavClub Checkout", `Price ID manquant pour le plan ${plan} (env: ${config.envKey})`);
      return new Response(
    ...
    console.log(`WavClub checkout session created: ${session.id} for plan=${plan} email=${email}`);
    ...
    console.error("WavClub checkout error:", error);
    await notifyError("WavClub Checkout", `Erreur: ${error.message}`);
    return new Response(
