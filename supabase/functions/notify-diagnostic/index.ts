...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!first_name || !email) {
      await notifyError("Diagnostic", "Champs obligatoires manquants");
    ...
    await notifySuccess("Diagnostic", `${first_name} ${last_name || ""} • ${email} • ${OFFER_LABELS[recommended_offer] || recommended_offer}`);
