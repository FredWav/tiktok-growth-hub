...
CREATE INDEX IF NOT EXISTS commerce_applications_rate ON public.commerce_applications(fingerprint, created_at);
...
CREATE OR REPLACE FUNCTION public.commerce_dates() RETURNS trigger LANGUAGE plpgsql SET search_path=public AS $$
BEGIN
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS commerce_order_dates ON public.commerce_orders;
...
ALTER TABLE public.wavacademy_subscriptions ADD COLUMN IF NOT EXISTS commerce_order_id uuid UNIQUE REFERENCES public.commerce_orders(id);

CREATE TABLE IF NOT EXISTS public.commerce_unmatched_payments (
...
CREATE TABLE IF NOT EXISTS public.commerce_mail (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE OR REPLACE FUNCTION public.commerce_record_payment(p_order uuid,p_method text,p_reference text,p_amount integer,p_received timestamptz,p_actor uuid DEFAULT NULL)
...
BEGIN
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS express_notification_outbox ON public.express_analyses;
...
CREATE OR REPLACE FUNCTION public.commerce_revenue() RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
